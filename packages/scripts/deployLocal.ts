...
// SCRIPT FINAL OTOMATIS - BISA UNTUK KAIROS & LOKAL
...
console.log("🔁 Running deploy script in TARGET_NETWORK=" + (hre.network.name));
...
async function main() {
  ...
  // =================================================================
  //  PENYESUAIAN OTOMATIS UNTUK JARINGAN LOKAL
  // =================================================================
  if (hre.network.name === "hardhat" || hre.network.name === "localhost") {
      console.log("🌐 Terdeteksi jaringan lokal (Hardhat). Menyesuaikan konfigurasi...");
      // Menggunakan akun kedua sebagai wallet ekosistem di jaringan lokal
      const localEcosystemWallet = signers[1].address; 
      
      console.log(`   -> Mengganti ECOSYSTEM_WALLET dari ${CONFIG.ECOSYSTEM_WALLET}`);
      console.log(`   -> menjadi ${localEcosystemWallet} (akun default Hardhat #1)`);
      CONFIG.ECOSYSTEM_WALLET = localEcosystemWallet;
      console.log("----------------------------------------------------");
  }
  ...
  const stakingRewardAmount = ethers.parseEther("10000000");
  const stakingUsdtRewardAmount = ethers.parseEther("5000000");
  const stakingKaiaRewardAmount = ethers.parseEther("5000000");
  ...
  if (!SKIP_LIQUIDITY) {
    ...
    if (!ecosystemSigner) {
        // Ini hanya akan berjalan di jaringan testnet asli, karena di lokal sudah ditangani di atas
